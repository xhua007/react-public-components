...
export default function App() {
  ...
  return (
	...
	const apiData: ApiPropItem[] = [
		...
		{ name: 'onLoadMore', desc: '触底加载下一页数据的回调函数', type: '() => void | Promise<void>', required: true },
		{ name: 'height', desc: '容器固定高度（传值时为局部滚动容器，不传时监听 window 全局滚动）', type: 'number | string', default: '-' },
		{ name: 'threshold', desc: '触发加载的触底距离阈值（像素）', type: 'number', default: '40' },
		{ name: 'endMessage', desc: '没有更多数据时的底部展示内容', type: 'ReactNode', default: "'已经到底啦 ~'" },
		{ name: 'loadingIndicator', desc: '自定义加载中指示器组件', type: 'ReactNode', default: 'Spinner + 文案' },
		{ name: 'scrollableTarget', desc: '指定外部自定义可滚动容器的 DOM id', type: 'string', default: '-' },
