...
export default function App() {
  return (
	...
	const apiData: ApiPropItem[] = [
		...
		{ name: 'sticky', desc: '是否开启智能吸顶 (position: sticky)', type: 'boolean', default: 'false' },
		{ name: 'fixed', desc: '是否开启全屏固定吸顶 (position: fixed)', type: 'boolean', default: 'false' },
		{ name: 'type', desc: "预设主题色系：'primary' | 'warning' | 'success' | 'danger' | 'purple'", type: 'AnnouncementType', default: "'primary'" },
		{ name: 'background', desc: '自定义横幅背景色或渐变色（传值时覆盖 type）', type: 'string', default: '-' },
		{ name: 'color', desc: '自定义文字颜色', type: 'string', default: "'#ffffff'" },
		{ name: 'top', desc: '吸顶时距离顶部的距离偏移（像素或 CSS 字符串）', type: 'number | string', default: '0' },
		...
		{ name: 'storageKey', desc: 'LocalStorage 记忆 Key（若传入则关闭后自动持久化记忆，不再重复弹出）', type: 'string', default: '-' },
		{ name: 'marquee', desc: '内容过长时是否开启跑马灯无缝平滑横向滚动', type: 'boolean', default: 'false' },
		...
		<div style={{ display: 'flex', flexDirection: 'column', gap: 32 }} key={resetKey}>
			...
			<div>
				...
				<p style={{ color: '#595959', fontSize: 14, marginBottom: 16 }}>
					向下滚动下方卡片内部的长列表，横幅会自动<strong>牢牢吸附锁定在容器最顶部</strong>（支持平滑关闭与 storageKey 状态记忆）。
				</p>
				...
				>
					...
					<div style={{ padding: '20px 24px', lineHeight: 2, color: '#595959' }}>
						...
						<p>2. 支持传入 <code>sticky</code> 属性实现智能吸顶锁定，传入 <code>storageKey</code> 实现用户点击关闭后持久化隐藏。</p>
			...
			<div>
				...
				<div style={{ display: 'flex', flexDirection: 'column', gap: 12, maxWidth: 720 }}>
					...
					<AnnouncementBar type="primary">
						<span>🚀 <b>版本升级：</b> react-public-components 现已包含 60+ 款通用核心组件！</span>
					</AnnouncementBar>

					{/* Purple 紫粉渐变 */}
					<AnnouncementBar type="purple">
						<span>🎉 <b>限时福利：</b> 欢迎 Star 支持我们的 GitHub 开源项目！</span>
					</AnnouncementBar>

					{/* Success 绿蓝渐变 */}
					<AnnouncementBar type="success">
						<span>✅ <b>健康状态：</b> 全球 12 个机房节点已全部恢复正常通信。</span>
					</AnnouncementBar>

					{/* 跑马灯滚动模式 */}
					<AnnouncementBar type="primary" marquee>
						<span>📢 <b>滚动快讯：</b> 今日全线系统运行稳定，欢迎大家提出宝贵的使用反馈与组件需求！</span>
					</AnnouncementBar>
				</div>
			</div>
