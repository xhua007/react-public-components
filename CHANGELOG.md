...
- **`LazyLoadBox`（视口懒加载容器）**：
  - 基于高性能 `IntersectionObserver` 深度解决多模块并发调用接口、非视口 DOM 过早挂载渲染导致的页面卡顿与网络带宽浪费问题。
  - 支持 `offset` 提前预加载距离配置（如距离视口 `200px` 时提前触发加载与挂载）。
  - 内置优雅的流光呼吸骨架屏与预估高度防塌陷（防止滚动条跳动）。
  - 支持 `once` 一次性挂载策略、局部滚动容器指定（`scrollContainer`）、强制加载（`forceRender`）与 Ref 命令式加载。

- **`PeriodSelect`（周期/粒度切换器）**：
  - 新增紧凑型时间周期与维度单选切换器，默认预设支持【周度、月度、季度、年度】。
  - 精准还原非第一项激活时的左侧高亮竖线分隔视觉效果。
  - 支持受控/非受控模式、三种尺寸（Small / Middle / Large）、自定义选项、自定义主题色（`activeColor`）、单项及全局禁用、块级等宽撑满（`block`）。
